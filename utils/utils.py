...
def calculate_signature_matrix_dataset(
    X: np.array, lags=[10, 30, 60], stride=1, num_timesteps=5
):
    max_lag = max(lags)
    ...
    for j in range(matrix_num):
        matrix_series = np.expand_dims(
            np.concatenate(result[j : j + num_timesteps], axis=0), axis=0
        )
...
def generate_harmonics(omega, t0, lmbda=0.3, length=2000):
    ...
    result = (
        (1 - alpha) * np.sin((t - t0) / omega)
        + alpha * np.cos((t - t0) / omega)
        + eps * lmbda
    )
...
def generate_dataset(n_features=30, seq_len=30000):
    ...
    dataset = [
        generate_harmonics(omega[i], t0[i], lmbda=0.1, length=seq_len)
